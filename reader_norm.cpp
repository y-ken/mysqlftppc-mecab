...
FtUnicodeNormalizerReader::FtUnicodeNormalizerReader(FtCharReader *internal, UNormalizationMode mode){
	this->mode = mode;
...
FtUnicodeNormalizerReader::~FtUnicodeNormalizerReader(){
	delete wrapper;
	delete normalizer;
}

bool FtUnicodeNormalizerReader::readOne(my_wc_t *wc, int *meta){
...
void FtUnicodeNormalizerReader::reset(){
	wrapper->reset();
	normalizer->reset();
	eos = false;
}
